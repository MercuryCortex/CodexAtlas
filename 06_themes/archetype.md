---
type: theme
id: "archetype"
name: "Archetype"
aka: ["archetypal image", "ur-Bild", "primordial image"]
traditions-where-attested: ["Platonism (forms-and-archetypes)", "Jungian depth psychology", "Eliade's comparative religion", "Joseph Campbell's monomyth"]
documents-attested: ["[[phase-3-002-plato-dialogues]]", "[[phase-7-017-psychology-and-alchemy]]", "[[phase-7-018-jung-aion]]"]
deities-associated: []
parent-theme: ""
child-themes: ["[[forms-and-archetypes]]", "[[individuation]]"]
status: "stub"
refs:
  - title: "The Archetypes and the Collective Unconscious (Collected Works vol. 9.1)"
    author: "Jung, C.G."
    year: 1959
    publisher: "Princeton University Press / Bollingen"
    type: "monograph"
    tier: 1
  - title: "Jung: A Biography"
    author: "Bair, Deirdre"
    year: 2003
    publisher: "Little, Brown"
    type: "monograph"
    tier: 2
  - title: "Re-Visioning Psychology"
    author: "Hillman, James"
    year: 1975
    publisher: "Harper & Row"
    type: "monograph"
    tier: 2
tags: [theme, jung, depth-psychology, platonism]
date-earliest: -380
dating-basis: B4
dating-basis-source: "\"Plato *Republic* 7.514a; Jung, C.G. 1934 'Über die Archetypen des kollektiven Unbewußten' *Eranos-Jahrbuch*\""
dating-basis-notes: "Platonic Forms c. -380 as proto-archetype; Jungian re-coinage 1934."
---

# Archetype

In Jung's depth psychology, archetypes are universal, inherited patterns of psychic structure — *not* specific images but the *form-giving capacities* that generate recurrent imaginal contents across cultures (the hero, the shadow, the anima/animus, the wise old man, the self). Jung distinguished sharply between the *archetype as such* (purely formal, never directly observable) and the *archetypal images* (the culturally varied symbols it produces).

Jung explicitly drew the term from Platonism and Augustine — archetypes function as the psychic analog of Platonic Forms (see [[forms-and-archetypes]]). Eliade's category of "archaic religion" presupposes a Jungian-inflected universalism of religious form. Joseph Campbell's "monomyth" (hero's journey) is a derivative concept.

- [[phase-7-017-psychology-and-alchemy]] — alchemical symbolism as archetypal projection
- [[phase-7-018-jung-aion]] — the Christ-archetype, the self
- Critics (Noll, Shamdasani) argue Jung's clinical evidence for inherited archetypal content is weak and the concept is unfalsifiable
