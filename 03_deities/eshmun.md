---
type: deity
id: "eshmun"
name: "Eshmun"
aka: ["Eshmoun", "ʾšmn", "Asclepius (Greek interpretatio)"]
tradition: "Phoenician (Sidonian)"
region: "Sidon (chief sanctuary at Bustan el-Sheikh); Carthage, Cyprus, Sardinia"
period-active-earliest: -1000
period-active-latest: 200
domains: ["healing", "youth/vitality", "tutelary god of Sidon"]
gender: "masculine"
role: "Phoenician god of healing; chief god of Sidon"
parent-of: []
child-of: []
consort: ["astarte-canaanite"]
attributes: ["serpent-entwined staff", "youth"]
attested-in: []
equivalents: ["[[asclepius-greek]]"]
syncretic-edges:
  - target: "[[asclepius-greek]]"
    type: "interpretatio-cultic"
    appearance-tier: 2
    role-in-context: "Eshmun-Asklepios of the Hellenized Phoenician coast"
    source: "Greeks consistently render Eshmun as Asklepios (serpent-staff healer); the Bustan el-Sheikh sanctuary becomes an Asklepieion. Lipiński 1995"
    source-tier: T1
  - target: "the-seven-kabeiroi"
    type: "scholarly-parallel"
    source: "Philo of Byblos / Damascius associate Eshmun with the Kabeiroi as the eighth/youngest brother"
    source-tier: T2
status: "metadata"
refs:
  - title: "Dieux et déesses de l'univers phénicien et punique"
    author: "Lipiński, Edward"
    year: 1995
    publisher: "Peeters (OLA 64)"
    type: "monograph"
    tier: 1
  - title: "The Oxford Handbook of the Phoenician and Punic Mediterranean"
    author: "Doak, B.R., and López-Ruiz, C. (eds.)"
    year: 2019
    publisher: "Oxford University Press"
    type: "reference"
    tier: 1
tags: [deity, phoenician, sidonian, canaanite, eshmun, healing, asclepius]
---

# Eshmun

Eshmun (Phoenician *ʾšmn*) is the principal god of **Sidon** and the great Phoenician deity of healing. His monumental sanctuary at Bustan el-Sheikh, northeast of Sidon, was a major Levantine healing-cult center from the 7th century BCE; King Eshmunazar II and Bodashtart of Sidon built and dedicated to him. As a god of restored health and youthful vitality, Eshmun is the Phoenician counterpart whom Greeks unfailingly identify with **[[asclepius-greek]]** (Asklepios), the serpent-staff healer — so completely that his Sidonian sanctuary functioned as an Asklepieion in the Hellenistic and Roman periods. A late tradition (Philo of Byblos; Damascius) makes Eshmun the eighth — the youngest and most beautiful son after the seven **Kabeiroi** — beloved of Astronoe/Astarte. His home is Sidonian-Phoenician; the Asklepios identification is a major (tier-2) Greek appearance-wire.
