---
type: deity
id: "atlas-titan"
name: "Atlas"
aka: ["Ἄτλας ('Endurer')"]
tradition: "Greek religion"
region: "Greece — by tradition the Atlas Mountains (NW Africa)"
period-active-earliest: -800
period-active-latest:
domains: ["sky-bearing", "the western edge of the world", "celestial-astronomy patron"]
gender: "masculine"
role: "Titan condemned to bear the heavens on his shoulders"
child-of: ["[[iapetus]]", "Clymene (Oceanid)"]
parent-of: ["the Pleiades", "the Hyades", "Hesperides (in some recensions)", "Calypso (in some recensions)"]
attributes: ["the celestial sphere on his shoulders", "the apples of the Hesperides (guarded in his orchard)"]
attested-in: ["[[phase-2-009-hesiod-theogony-works-and-days]] 517–520", "Apollodorus *Library* 2.5.11 (Heracles and the apples of the Hesperides)", "[[phase-3-022-plato-timaeus-critias-atlantis]] (eponym of Atlantis)"]
equivalents: ["[[purusha]] (cosmic-pillar function)"]
syncretic-edges:
  - target: "[[purusha]]"
    type: "scholarly-parallel"
    source: "Cosmic-pillar — primordial figure who supports / is the universe"
  - target: "[[ymir]]"
    type: "scholarly-parallel"
    source: "Cosmic-frame primordial body"
status: "metadata"
refs:
  - title: "Hesiod: Theogony, Works and Days, Shield"
    author: "Athanassakis, Apostolos N. (trans.)"
    year: 2004
    publisher: "Johns Hopkins UP"
    tier: 1
  - title: "Plato: Timaeus and Critias"
    author: "Lee, Desmond (trans.)"
    year: 1971
    publisher: "Penguin Classics"
    tier: 1
tags: [deity, greek, titan, sky-bearer, atlantis-eponym]
---

# Atlas

## Identity
Titan son of [[iapetus]], brother of [[prometheus]] and Epimetheus. After the Titanomachy he is **condemned by Zeus to bear the heavens on his shoulders** (Hesiod *Theogony* 517–520) at the far western edge of the world — the position the Greeks identified with the great mountain range across the strait from Iberia, hence the name *Atlas Mountains* (NW Africa). His daughters: the seven Pleiades, the Hyades, and in some recensions the Hesperides whose golden apples Heracles wins for his eleventh labor. **Plato's *Timaeus-Critias***: the first king of **[[phase-3-022-plato-timaeus-critias-atlantis|Atlantis]]** is named Atlas, son of Poseidon and Cleito (not the Titan but eponymous of him), making Atlantis = "land of Atlas."

## Cross-tradition identifications
- **[[purusha]]** — *scholarly-parallel*: cosmic-frame figure.
- **[[ymir]]** — *scholarly-parallel*: cosmic body forming the frame of the world.

## Refs
1. Athanassakis (trans.), *Hesiod* (Johns Hopkins UP, 2004) — Tier 1.
2. Lee (trans.), *Timaeus and Critias* (Penguin, 1971) — Tier 1.
