---
type: deity
id: "fortuna"
name: "Fortuna"
aka: ["Tyche (Greek)", "Fors Fortuna", "Fortuna Primigenia (Praeneste)", "Fortuna Redux"]
tradition: "Roman religion (with Hellenistic Tyche substrate)"
region: "Rome — major cult-center at Praeneste (Palestrina, Lazio)"
period-active-earliest: -500
period-active-latest:
domains: ["fate", "chance", "fortune", "civic luck", "personal destiny"]
gender: "feminine"
role: "goddess of fate / fortune / chance"
attributes: ["wheel (Rota Fortunae)", "ship's rudder (steering destinies)", "cornucopia (her gifts)", "globe (her standing surface — unstable footing)", "blindfold (later development)"]
attested-in: ["Plautus, Terence", "Cicero *De Fato*", "Pliny *NH* 2.22", "Boethius *Consolation of Philosophy*"]
equivalents: ["[[tyche-greek]]", "[[the-moirai]]", "[[lakshmi]]"]
syncretic-edges:
  - target: "[[tyche-greek]]"
    type: "interpretatio-cultic"
    source: "Champeaux 1982-1987, Fortuna (2 vols.); Beard/North/Price 1998 Religions of Rome vol. 1"
    source-tier: T1
    notes: "interpretatio-romana with shared cult: Greek Tyche imported wholesale into Roman religion at the level of cult-practice. The Praeneste cult of Fortuna Primigenia predates and influences imperial-era Hellenization — the relationship is older than a simple name-mapping and involves the actual sanctuary cult"
  - target: "[[the-moirai]]"
    type: "scholarly-parallel"
    source: "Champeaux 1982-1987"
    source-tier: T1
    notes: "Substitutive at the Hellenistic moment — Fortuna takes over functions previously divided among the Moirai (apportionment of fate)"
  - target: "[[lakshmi]]"
    type: "scholarly-parallel"
    source: "Comparative religion — feminine personification of variable fortune; the unsteady divine gift; no historical transmission claim"
    source-tier: T1
    notes: "Structural parallel — both Lakshmi and Fortuna personify the FEMININE-GIFT-OF-PROSPERITY divine role; this is NOT a transmission claim, only a typological one"
status: "metadata"
refs:
  - title: "Religions of Rome, Vol. 1"
    author: "Beard, Mary, North, John & Price, Simon"
    year: 1998
    publisher: "Cambridge UP"
    tier: 1
  - title: "Fortuna: Le culte de la Fortune à Rome et dans le monde romain"
    author: "Champeaux, Jacqueline"
    year: 1982
    publisher: "École française de Rome"
    tier: 1
  - title: "Boethius: The Consolation of Philosophy"
    author: "Tester, S.J. (trans.)"
    year: 1973
    publisher: "Loeb Classical Library"
    tier: 1
tags: [deity, roman, fate, fortune, wheel, civic-cult]
---

# Fortuna

## Identity
The Roman goddess of **fate, fortune, and chance** — and the principal Roman attempt to **personify what cannot be controlled**. Her great cult-center at **Praeneste** (modern Palestrina, Lazio) was a massive terraced sanctuary built into a hillside (~120 BCE) where her oracle was consulted by oak-tablet *sortes* (lots) drawn by a child — among the most important Italian oracle-sites alongside Cumae and Dodona. **Servius Tullius**, the sixth king of Rome, was credited with introducing her cult (traditionally ~570 BCE).

## Mythological role
**Rota Fortunae (Wheel of Fortune)**: the iconographic image of Fortuna with a wheel becomes — via **Boethius's *Consolation of Philosophy*** (early 6th c. CE, written in prison awaiting execution) — the great medieval Christian moral image of the unreliability of worldly success. Boethius's Lady Philosophy explicitly *demotes* Fortuna and her wheel beneath the Christian Providence — but the wheel survives Christianization and becomes one of the most-depicted motifs in medieval manuscripts and cathedral rose-windows (Amiens, Beauvais, Basel).

## Cross-tradition identifications
- **Tyche** — *ancient-identification*: Greek cognate.
- **[[the-moirai]]** — *parallel-motif*: rival principle of fate.
- **[[lakshmi]]** — *scholarly-parallel*: feminine-divine variable-fortune.
- **Boethian Providence** — *polemic-inversion*: Christian re-subordination of pagan Fortune.

## Refs
1. Beard, North & Price, *Religions of Rome* vol. 1 (Cambridge UP, 1998) — Tier 1.
2. Champeaux, *Fortuna* (École française de Rome, 1982) — Tier 1.
3. Boethius, *Consolation of Philosophy* (Loeb) — Tier 1.
