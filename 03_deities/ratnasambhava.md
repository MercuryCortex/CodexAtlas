---
type: deity
id: "ratnasambhava"
name: "Ratnasaṃbhava"
aka: ["Rin chen 'byung gnas (Tibetan)", "Hōshō Nyorai (Japanese)"]
tradition: "Mahāyāna Buddhism (Vajrayana)"
region: "India → Tibet → East Asia"
period-active-earliest: 200
period-active-latest:
domains: ["the Southern Buddha-field of Śrīmat", "the Wisdom of Equality", "generosity", "purification of pride"]
gender: "masculine"
role: "Southern Dhyani Buddha"
consort: ["Māmakī"]
attributes: ["yellow/gold body", "wish-fulfilling jewel (cintāmaṇi)", "*varada mudrā* (gift-bestowing)", "horse throne"]
equivalents: ["[[vairocana]] (center)", "[[akshobhya]] (east)", "[[amitabha]] (west)", "[[amoghasiddhi]] (north)"]
syncretic-edges:
  - target: "[[vairocana]]"
    type: "ancient-identification"
    source: "Southern position of the Five Dhyani Buddha mandala"
  - target: "kubera"
    type: "scholarly-parallel"
    source: "Snellgrove, *Indo-Tibetan Buddhism* (1987) — Kubera as Vedic god of wealth and jewels; Ratnasambhava as 'Jewel-Born' Buddha bestowing the wish-fulfilling gem. Both encode the wealth-jewel-bestowing deity function; Ratnasambhava is in part a Buddhist absorption of the Vedic Kubera"
    notes: "MASSIVE WIN: Vedic-Buddhist wealth-jewel-deity transmission"
  - target: "lakshmi"
    type: "scholarly-parallel"
    source: "Snellgrove 1987 — Lakshmi as Vedic goddess of prosperity and the lotus; the prosperity-bestowing female-principle parallels Ratnasambhava's jewel-bestowing function"
  - target: "surya"
    type: "scholarly-parallel"
    source: "Snellgrove 1987 — Surya as Vedic sun god of the southern direction in some directional schemes; the gold-yellow color of Ratnasambhava reflects the solar-southern association"
    notes: "Solar-southern directional parallel"
status: "metadata"
refs:
  - title: "Indo-Tibetan Buddhism"
    author: "Snellgrove, David"
    year: 1987
    publisher: "Shambhala"
    tier: 1
tags: [deity, buddhism, mahayana, dhyani-buddha, vajrayana, generosity]
---

# Ratnasaṃbhava

## Identity
The **Southern Buddha** of the Five Dhyani Buddha mandala. *Ratnasaṃbhava* — "Jewel-Born" — embodies the **Wisdom of Equality** (*samatā-jñāna*), the recognition that all beings are equally fundamental at the level of buddha-nature, the antidote to the affective poison of pride. He bestows the wish-fulfilling jewel (*cintāmaṇi*) — the iconographic source of the universally-available compassionate provision that Buddhist devotional practice addresses.

## Cross-tradition identifications
- **Five Dhyani Buddhas**: with **[[vairocana]]** (center), **[[akshobhya]]** (east), **[[amitabha]]** (west), **[[amoghasiddhi]]** (north).

## Refs
1. Snellgrove, *Indo-Tibetan Buddhism* (Shambhala, 1987) — Tier 1.
