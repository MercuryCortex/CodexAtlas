---
id: chaac
title: "Chaac"
type: deity
status: metadata
tier: 2
tradition: tradition-maya-religion
date_earliest: -300
date_latest: 900
region: "Classic Maya civilization (Yucatan Peninsula, Guatemala, Belize, Chiapas)"
family: Maya
gender: male
domain: ["rain", "lightning", "water", "agriculture", "fertility"]
tags:
  - maya-deity
  - rain-deity
  - storm-deity
  - agriculture
syncretic-edges:
  - target: "tlaloc"
    type: "scholarly-parallel"
    source: "Coe & Houston, *The Maya* (2015); Freidel, Schele, Parker, *Maya Cosmos* (1993) — Chaac and Tlaloc are the Maya and Aztec instances of the central-Mesoamerican rain-storm-deity; both have goggle-eye iconography and quadripartite (one per cardinal direction) elaboration"
    source-tier: T1
    notes: "Mesoamerican rain-deity convergence"
  - target: "indra"
    type: "scholarly-parallel"
    source: "Eliade, *Patterns in Comparative Religion* (1958), Ch. II — Indra as Vedic storm-thunder-axe deity; Chaac as Maya storm-lightning-axe deity. The thunder-axe wielding storm-god is among the most widespread deity types"
    source-tier: T1
    notes: "Storm-god-with-axe cross-cultural archetype"
  - target: "baal-hadad"
    type: "scholarly-parallel"
    source: "Eliade 1958 — Canaanite Baal as storm-god with thunderbolt; structural parallel to Chaac"
    source-tier: T1
  - target: "zeus"
    type: "scholarly-parallel"
    source: "Eliade 1958; the thunderbolt-wielding sky-storm-god as supreme-of-pantheon (Zeus, Indra, Thor, Perun, Chaac) is a near-universal Indo-European-and-beyond deity type"
    source-tier: T1
  - target: "thor"
    type: "scholarly-parallel"
    source: "Eliade 1958; Norse Thor's Mjölnir parallels Chaac's lightning-axe"
    source-tier: T1
  - target: "perun"
    type: "scholarly-parallel"
    source: "Eliade 1958; Slavic Perun as storm-god parallels Chaac structurally"
    source-tier: T1
    notes: "MASSIVE WIN: 6-tradition storm-deity-with-thunder-axe/thunderbolt convergence (Maya Chaac, Aztec Tlaloc, Vedic Indra, Canaanite Baal, Greek Zeus, Norse Thor, Slavic Perun) — the storm-axe-as-rain-bringer is one of the most-universally-attested mythological constellations"
refs:
  - "Simon Martin and Nikolai Grube, Chronicle of the Maya Kings and Queens, Thames and Hudson, 2000 [T1/T2]"
  - "David Freidel, Linda Schele, Joy Parker, Maya Cosmos: Three Thousand Years on the Shaman's Path, Morrow, 1993 [T2]"
dating-basis: B3
dating-basis-source: "\"Coe, M. 2011 *The Maya*\""
dating-basis-notes: "Chaac Maya rain-deity Preclassic to Postclassic; iconographically attested c. -200 (long-nosed god)."
---

# Chaac

Chaac (also Chac, Chahk) is the ancient Maya god of rain, lightning, storms, and water — one of the most important and widely worshipped deities in the Maya pantheon. He is iconographically distinctive: long, curling nose (sometimes depicted as a serpent); axe or lightning-bolt as his weapon; goggle-like eyes.

**Function**: As the deity controlling rainfall, Chaac was central to Maya agricultural religion. Maya cities frequently featured rain rituals addressed to Chaac, particularly in the Yucatan where surface water is scarce and rainfall is existentially important for agriculture. The Puuc-region Maya architecture (sites including Uxmal, Kabah, Sayil, Labna) features extensive Chaac masks on building facades — thousands of stone rain-deity faces integrated into the architectural program.

**Multiplicity**: Chaac often appears in groups of four (one for each cardinal direction) — the four Chaacs correspond to the four directional rain-winds. This quadripartite structure mirrors other fundamental Maya cosmological organizing principles.

**Relationship to Classic Maya script**: Chaac is extensively documented in Classic Maya inscriptions; his glyph-head is one of the most recognizable and fully deciphered head-variant signs in the Maya script system. See [[alphabet-mayan-glyphs]] for the decipherment context.

**Cross-tradition parallels**: Storm/rain deity wielding a lightning bolt — parallel forms include [[tlaloc]] (Aztec rain god, with similar goggle-eye iconography), [[indra]] (Vedic storm deity wielding thunderbolt), [[zeus]] (Greek storm deity), [[thor]] (Norse storm deity). The rain-deity wielding lightning is among the most widespread deity archetypes in world religion.
