---
type: deity
id: "thor"
name: "Þórr"
aka: ["Thor", "Þunor (Old English)", "Donar (continental Germanic)", "Ásaþórr"]
tradition: "Norse / Germanic"
region: "Scandinavia, Anglo-Saxon, continental Germanic"
period-active-earliest: -100
period-active-latest: 1100
domains: ["thunder", "lightning", "storm", "agricultural fertility (storms bring rain)", "protection (of humans against giants)"]
gender: "masculine"
role: "thunder-god / defender of Midgard"
parent-of: []
child-of: ["[[odin]] and Jörð (Earth)"]
consort: ["Sif"]
attributes: ["hammer *Mjölnir*", "iron gloves *Járngreipr*", "belt of strength *Megingjörð*", "goat-drawn chariot (*Tanngrisnir* and *Tanngnjóstr*)", "red beard"]
attested-in: ["[[phase-8-014-poetic-edda]]", "[[phase-8-015-prose-edda-snorri]]"]
equivalents: ["[[jupiter]]", "[[zeus]]", "[[indra]]", "[[perun]]", "[[taranis]]", "[[susanoo]]", "[[raijin]]", "[[baal-hadad]]", "[[marduk]]", "[[perkunas]]", "[[chaac]]", "[[chac]]", "[[daramulan]]", "[[leigong]]", "[[reue-lusitanian]]", "[[shango]]", "[[takemikazuchi]]", "[[tawhirimatea]]", "[[tishtrya]]", "[[vahagn]]"]
syncretic-edges:
  - target: "[[jupiter]]"
    type: "interpretatio-romana"
    source: "Lindow 2001; weekday-calque Thursday/Þórsdagr = Jovis dies"
    notes: "Roman interpretatio germanica identifies the Germanic thunderer with Jupiter. The Thursday-calque is the load-bearing evidence."
  - target: "[[zeus]]"
    type: "interpretatio-graeca"
    source: "Via Jupiter — Zeus-Jupiter pair is classical"
    notes: "Through the Greek-Latin Zeus-Jupiter identity, the interpretatio reaches Zeus transitively"
  - target: "[[indra]]"
    type: "cognate"
    source: "West 2007 ch. 6; Mallory & Adams 2006 (Proto-Indo-European *Perkʷūnos thunderer)"
    notes: "Proto-Indo-European thunder-god *Perkʷūnos cluster — same divine type across IE branches"
  - target: "[[perun]]"
    type: "cognate"
    source: "West 2007 ch. 6"
    notes: "PIE *Perkʷūnos — Slavic Perun is the closest etymological cognate to the *Perkʷūnos root"
  - target: "[[taranis]]"
    type: "cognate"
    source: "West 2007 ch. 6 — Celtic thunderer parallel"
    notes: "PIE thunder-god cluster — Celtic instance"
  - target: "[[susanoo]]"
    type: "parallel-motif"
    source: "Raijin.md notes Susanoo-Indra-Thor-Perun-Shango cluster"
    notes: "Non-PIE storm-god parallel from Shinto tradition — same divine type, independent transmission"
  - target: "[[raijin]]"
    type: "parallel-motif"
    source: "Shinto thunder-deity"
    notes: "Best-wired thunder-god in the corpus; reciprocal back-link"
  - target: "[[baal-hadad]]"
    type: "parallel-motif"
    source: "Smith 1994 *The Ugaritic Baal Cycle*"
    notes: "Semitic non-PIE storm-warrior parallel"
  - target: "[[marduk]]"
    type: "parallel-motif"
    source: "Lambert 2013 *Babylonian Creation Myths*"
    notes: "Mesopotamian storm-warrior + chaoskampf parallel"
status: "metadata"
refs:
  - title: "Old Norse Myths, Literature and Society"
    author: "Lindow, John"
    year: 2001
    tier: 1
tags: [norse, germanic, thor, thunder, hammer, phase-8]
---

# Þórr

## Identity
Thunder-god of Norse and broader Germanic religion. Where Óðinn was the god of warriors-elite, kings, and the educated, Þórr was the god of *free farmers* — the broader Norse population's principal deity. Place-names with Þórr-elements (Tórshavn, Thorsten, etc.) and personal names (Þórsteinn, Þorgeirr) are far more numerous than Óðinn-elements; archaeological evidence (hammer-amulets, especially in the late Viking Age as Christian conversion advanced) shows Þórr was the popular cult-figure of late paganism. He defends gods and humans against the giants (*jötnar*) and is the principal active-protective deity.

## Earliest attestation
Tacitus *Germania* 9 identifies the Germans' "Hercules" — this is Þunor/Þórr. Bronze Age Scandinavian rock-art may show storm-god figures with hammers / axes. Old English *Þunor*; continental *Donar* (visible in *Donnerstag* "Thursday"). Old Norse literary attestation from skaldic poetry onward.

## Mythological role
Thor's principal mythic activity is fighting giants. The most famous narratives: the *Þrymskviða* (Thor recovers his hammer by cross-dressing as Freyja); the Útgarða-Loki episode (Thor humiliated by deceptive giant-magic); the Hymir-fishing-for-Miðgarðsormr (Thor catches the Midgard-Serpent, the central cosmic-combat scene, narrowly fails to kill it). At Ragnarǫk, Thor kills the Midgard-Serpent but dies from its poison.

## Cross-tradition identifications
- **Jupiter / Zeus / [[indra]] / [[perun]] / Perkūnas** — *scholarly-parallel*: Indo-European storm-god comparative complex; Thor is the Germanic instantiation of a deep IE pattern. Perun is the Slavic cognate (cf. *Primary Chronicle* Rus'-Byzantine treaty oaths "by Perun their god"); Perkūnas is the Baltic cognate. See [[perun]] for the Slavic reconstruction.
- **"Hercules"** (Tacitus) — *ancient-identification*: Roman interpretatio of Germanic Thor as Hercules on the basis of strength + monster-fighting.

## Iconography
The hammer *Mjölnir* — the most distinctive attribute; goat-chariot; red beard; iron gloves and belt of strength. Hammer-amulets (Mjölnir pendants) survive in considerable archaeological number, especially from the conversion period (a counter-symbol to the cross).

## Refs
1. Lindow, J. (2001). *Old Norse Myths, Literature and Society*. Oxford UP.
2. Dronke, U. (1969–2011). *The Poetic Edda*. Oxford UP.
3. Faulkes, A. (1995). *Edda* (Snorri). Everyman.
4. Perkins, R. (2001). *Thor the Wind-Raiser and the Eyrarland Image*. Viking Society.
