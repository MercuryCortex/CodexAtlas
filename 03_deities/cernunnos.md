---
type: deity
id: "cernunnos"
name: "Cernunnos"
aka: ["the Horned One", "the Gundestrup-cauldron figure"]
tradition: "Celtic paganism (Gallo-Roman + Insular substrate)"
region: "Gaul (chief evidence), Britain, the Gundestrup cauldron (found Denmark, Celtic-Thracian workmanship c. -150 to -50)"
period-active-earliest: -400
period-active-latest:
domains: ["antlered nature", "wild animals", "abundance", "the otherworld passage", "the chthonic underworld"]
gender: "masculine"
role: "antlered nature-god / lord-of-the-animals (potnios theron)"
attributes: ["stag antlers", "torc (Celtic neck-ring) — sometimes held in hand, sometimes worn", "ram-headed serpent", "seated cross-legged in meditation-pose"]
attested-in: ["Pillar of the Boatmen (Paris, ~14–37 CE — explicit *CERNVNNOS* inscription)", "Gundestrup Cauldron (c. -150 to -50, Denmark)", "Reims relief", "Saintes inscription"]
equivalents: []
syncretic-edges:
  - target: "[[pan-greek]]"
    type: "structural-parallel"
    notes: "Both horned wild-nature gods who preside over animals and the untamed landscape; both associated with sexuality and fertility; both inspire the 'panic' of the wild. Green (1986) and Detienne (1989) discuss the parallel."
  - target: "[[shiva]]"
    type: "structural-parallel"
    notes: "MASSIVE WIN: the three-way horned-lord-of-animals cluster. Cernunnos ↔ Pashupati ↔ Shiva. Shiva's title Paśupati ('Lord of Beasts') is the Sanskrit equivalent of Cernunnos's *Potnios Theron* ('Lord of Animals'). The Pashupati seal at Mohenjo-Daro (c. -2500) shows a cross-legged, antlered/horned figure surrounded by animals in an iconography visually identical to Cernunnos on the Gundestrup Cauldron — and Pashupati is identified as a proto-Shiva by most scholars (Marshall 1931; Chakrabarti 1979). The chain Pashupati → Shiva-Pashupati → Cernunnos: three independently developed instantiations of the horned lord-of-animals archetype, separated by 4,000 years and two hemispheres. Transmission is NOT demonstrable per vault methodology — this is `parallel-form`."
status: "metadata"
refs:
  - title: "The Gods of the Celts"
    author: "Green, Miranda"
    year: 1986
    publisher: "Sutton"
    tier: 1
  - title: "Animals in Celtic Life and Myth"
    author: "Green, Miranda"
    year: 1992
    publisher: "Routledge"
    tier: 1
tags: [deity, celtic, gallic, horned-god, lord-of-animals, gundestrup, otherworld]
---

# Cernunnos

## Identity
Antlered nature-god of the Celts — the cross-legged **horned figure** holding a torc and a ram-headed serpent who appears in the central panel of the **Gundestrup Cauldron** (c. -150 to -50 BCE, the most spectacular Celtic ritual vessel ever found, discovered in a Danish bog in 1891). His name appears only once epigraphically — on the **Pillar of the Boatmen** (Paris, ~14–37 CE), where the inscription *CERNVNNOS* glosses an antlered head — but the iconography is widespread across Gaul, Britain, and the Celtic-Thracian frontier.

## Mythological role
**Lord-of-the-animals** (*Potnios Theron*) — Cernunnos is depicted surrounded by stags, bulls, ram-headed serpents, hounds, and other wild creatures, an iconography running back to Minoan Master-of-Animals figures (Knossos, c. -1600) and the Indus Valley Pashupati seal (Mohenjo-Daro, c. -2500). His posture — **cross-legged, palms-on-knees** — is shared with both of those earlier figures; the visual cognacy is striking, but transmission is **not demonstrable** (per vault methodology, this is `parallel-form` not `ancestor-of`).

## Cross-tradition identifications
- **[[pan-greek]]** — *parallel-motif*: horned wild-nature god.
- **Pashupati seal** — *parallel-form* (NOT *ancestor-of*; per [[methodology]] symbol-research discipline): visual cognacy without documented transmission.

## Iconography
The single most distinctive Celtic divine iconography: stag-antlered, cross-legged, holding a torc (Celtic status-marker) and a horned/ram-headed serpent (uniquely Celtic motif).

## Refs
1. Green, *The Gods of the Celts* (Sutton, 1986) — Tier 1.
2. Green, *Animals in Celtic Life and Myth* (Routledge, 1992) — Tier 1.
