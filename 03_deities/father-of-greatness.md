---
type: deity
id: "father-of-greatness"
name: "Father of Greatness"
aka: ["Zurvan (in Iranian Manichaean adaptations)", "Pater Magnitudinis (Latin)", "Aba d-Rabbutha (Aramaic)"]
tradition: "Manichaean"
region: "Mesopotamia (Mani's origin); later transmitted across Roman, Iranian, Central Asian, and Chinese contexts"
period-active-earliest: 240
period-active-latest: 1400
domains: ["supreme Light", "primal divinity", "co-eternal counterpart to the Prince of Darkness"]
gender: "masculine"
role: "the supreme God of Light in Manichaean cosmic dualism"
parent-of: ["the five emanations: Friend of the Lights, Great Builder, Living Spirit (with his five sons), Mother of Life, Primal Man"]
attested-in: ["[[phase-4-014-cologne-mani-codex]]", "[[phase-4-015-kephalaia-of-the-teacher]]", "[[phase-4-016-shabuhragan]]"]
equivalents: ["Zurvan (Mani's deliberate Iranian-language equivalent for missionary purposes)"]
syncretic-edges:
  - target: "zurvan"
    type: "ancient-identification"
    source: "Mani's own use in Šābuhragān; cf. Boyce & Grenet 1991"
  - target: "theme-manichaeism-synthesis"
    type: "supreme-deity-of-syncretic-religion"
    source: "Tardieu 2008 *Manichaeism*; Lieu 1992 *Manichaeism in the Later Roman Empire and Medieval China*; BeDuhn 2000 *The Manichaean Body*; Reeves 1992 — the Father of Greatness is the supreme-Light pole of Mani's deliberately syncretic two-principle cosmology (Zurvanite Zoroastrian cosmic dualism + Buddhist enlightenment-teacher prophet-chain + Christian Paraclete-claim + Mesopotamian astrological framework). His Zurvanite-derived co-eternal-equal-with-Darkness structure is the structural skeleton of the *first global religious synthesis*. See [[theme-manichaeism-synthesis]]"
    notes: "Cross-lens: 03_deities → 06_themes. Hub also lists [[mani]] + [[zarathustra]] + [[siddhartha-gautama-buddha]] + [[augustine-of-hippo]] (04_persons); [[tradition-manichaeism]] + [[tradition-zoroastrianism]] + [[tradition-buddhism]] + [[tradition-christianity-canonical]] + [[tradition-catharism]] (07_traditions); [[phase-4-014-cologne-mani-codex]] + [[phase-4-015-kephalaia-of-the-teacher]] + [[phase-4-016-shabuhragan]] (02_documents) — those lens-back-edges still pending"
  - target: "third-century-collision-zone"
    type: "Manichaean-pole-of-3rd-c-synthesis-window"
    source: "BeDuhn 'The Co-formation of the Manichaean and Zoroastrian Religions'; Tardieu 2008 — Mani's Father-of-Greatness is the Manichaean-tradition contribution to the 3rd-century collision zone (Mani founded the religion ~240 CE, was executed ~274–277 CE — squarely in the 200–280 CE window). Together with Plotinus's One + Origen's Christian Logos + Sasanian Zoroastrianism's Ahura Mazda, this is one of the four supreme-deity articulations that the collision-zone is producing simultaneously across Roman + Persian + Mesopotamian + Egyptian intellectual networks. See [[third-century-collision-zone]]"
    notes: "Cross-lens: 03_deities → 06_themes. The simultaneous emergence of four supreme-deity articulations across four traditions in 80 years is the collision-zone hub's structural finding — the deity-end documents this convergence"
status: "metadata"
refs:
  - title: "Mani and Manichaeism"
    author: "Lieu, Samuel N.C."
    year: 1992
    publisher: "Manchester UP"
    type: "monograph"
    tier: 1
  - title: "The Kephalaia of the Teacher"
    author: "Gardner, Iain"
    year: 1995
    publisher: "Brill"
    type: "primary-translation"
    tier: 1
tags: [deity, manichaean, father-of-greatness, dualism, light]
---

# Father of Greatness

The supreme deity of Manichaean theology — the personification of the Realm of Light, co-eternal with the Prince of Darkness across the boundary that the rigorously dualistic Manichaean cosmology posits. The Father dwells in his five dwellings of intelligence, knowledge, reason, thought, and consideration, and emanates a series of five emanations (Friend of the Lights, Great Builder, Living Spirit, Mother of Life, Primal Man) to respond to the Darkness's invasion of the borderlands. In the Iranian Manichaean register, Mani himself substitutes Zurvan as the Father's name — a deliberate missionary equivalence designed to make Manichaeism legible to Zoroastrian audiences (though Zoroastrian theology proper does not endorse Zurvanism's primacy of Zurvan).
