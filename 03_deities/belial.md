---
type: deity
id: "belial"
name: "Belial"
aka: ["Beliar", "the angel of darkness", "the prince of malice"]
tradition: "Israelite religion / Second Temple apocalyptic / Qumran"
region: "Levant"
period-active-earliest: -700
period-active-latest: 200
domains: ["evil", "lying spirit", "eschatological adversary"]
gender: "masculine"
role: "evil-spirit | adversary"
parent-of: []
child-of: []
consort: []
attributes: ["worthlessness (the etymology: *beliyya'al* = without-worth)", "darkness", "the wicked lot"]
attested-in: ["[[phase-2-011-hebrew-bible-d-source]]", "[[phase-3-011-dead-sea-scrolls]]", "[[phase-3-015-pauline-epistles]]"]
equivalents: []
syncretic-edges:
  - target: "angra-mainyu-ahriman"
    type: "scholarly-parallel"
    source: "Collins 1998 *The Apocalyptic Imagination*; Russell 1977 *The Devil*; Alexander 2010 — Belial in the Dead Sea Scrolls (especially the Community Rule 1QS 3:13–4:26 Two-Spirits tractate + the War Scroll 1QM) is the cosmic-adversary commander of the Army of Darkness opposing the Prince of Light. The structural parallel with Zoroastrian Angra Mainyu commanding the daēvas is the closest documented Iranian-influence case in Qumran sectarian writings"
    source-tier: T1
    notes: "Cross-lens: 03_deities → 03_deities"
  - target: "satan-christian"
    type: "ancestor-of"
    source: "Russell 1977; Pagels 1995 *The Origin of Satan*; Forsyth 1987 *The Old Enemy* — Belial in 2 Cor 6:15 ('what concord hath Christ with Belial?') is Paul's only use of the name; subsequent Christian tradition absorbs Belial as one name among many for the cosmic adversary later unified as Satan"
    source-tier: T1
  - target: "theme-satan-angra-mainyu-transfer"
    type: "intermediate-figure-of-transfer"
    source: "Collins 1998; Russell 1977; Alexander 2010; Forsyth 1987 — Belial is the *intermediate* form of the cosmic-adversary in the transformation pre-exilic *ha-satan* (court functionary) → Qumran Belial (cosmic-army commander) → New Testament Satan (universal adversary of God's purposes). The Qumran sectarian Belial-figure is the missing-link between the Hebrew Bible's prosecutorial-satan and the developed Christian Satanology. See [[theme-satan-angra-mainyu-transfer]]"
    source-tier: T1
    notes: "MASSIVE-WIN: Belial is the deity-end documentary evidence of the Second-Temple transformation; the Qumran community's Belial-vs-Prince-of-Light cosmology IS the Zoroastrian dualism transposed into Jewish-sectarian vocabulary. Cross-lens: 03_deities → 06_themes"
status: "metadata"
refs:
  - title: "The Apocalyptic Imagination"
    author: "Collins, John J."
    year: 1998
    publisher: "Eerdmans"
    type: "monograph"
    tier: 1
  - title: "The Origin of Satan"
    author: "Pagels, Elaine"
    year: 1995
    publisher: "Random House"
    type: "monograph"
    tier: 1
tags: [deity, demon, qumran, second-temple, satan-adjacent]
---

# Belial

## Identity
In the Hebrew Bible, *beliya'al* is initially a *common noun* meaning "worthlessness" or "wickedness" — "sons of Belial" (Deut 13:13, 1 Sam 2:12, etc.) means "worthless men, troublemakers." By the Second Temple period, *Belial* has been *personified* into the supreme evil-spiritual figure: the leader of the Sons of Darkness in Qumran's War Scroll (1QM), the chief adversary in the Two Spirits discourse, the head of the dominion of darkness in the present age until eschatological judgment. In the Pauline corpus, 2 Cor 6:15 has *Beliar* as the foil for Christ ("what concord has Christ with Belial?"). The figure overlaps with but is not identical to *Satan* / *the devil* / *Azazel* / *Mastema* — Second Temple Jewish demonology produces a small cast of overlapping adversarial figures with shifting names.

## Earliest attestation
- [[phase-2-011-hebrew-bible-d-source]] — Deut 13:13 *bene-beliya'al*.
- [[phase-3-011-dead-sea-scrolls]] — the *Community Rule* Two-Spirits discourse (1QS III–IV), the *War Scroll* (1QM), and the *Damascus Document* (CD) all make Belial the supreme present-age adversary.
- [[phase-3-015-pauline-epistles]] — 2 Cor 6:15 *Beliar* as the antithesis of Christ.

## Mythological role
- Leader of the spirits of darkness in the dualist Qumran framework.
- Eschatological adversary destined for defeat at the end-time war.

## Cross-tradition identifications
The Qumran-period overlap with [[satanael]], Mastema, and the later figure of Satan is significant but not identifying; Belial is one figure in a cast.

## Refs
1. Collins, *The Apocalyptic Imagination*, 1998.
2. Pagels, *The Origin of Satan*, 1995.
