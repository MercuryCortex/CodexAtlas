---
type: deity
id: "ahura-mazda"
name: "Ahura Mazda"
aka: ["Ohrmazd (Middle Persian / Pahlavi)", "Hormazd", "Auramazdā (Old Persian)"]
tradition: "Zoroastrian"
region: "Iran / Central Asia"
period-active-earliest: -1200
period-active-latest: 700
domains: ["wisdom", "truth (aša)", "creation", "supreme deity"]
gender: "masculine (transcendent / non-anthropomorphic in Gathic theology)"
role: "supreme god; uncreated creator; principle of truth and good"
parent-of: ["[[asha-vahishta]] (the first Amesha Spenta)", "[[vohu-manah]] (the second Amesha Spenta)", "[[atar]] (the divine fire)", "Spenta Mainyu (the Bounteous Spirit, in some accounts identified with Ahura Mazda himself)"]
child-of: []
consort: ["Spənta Ārmaiti (Holy Devotion, an Amesha Spenta) in some traditions"]
attributes: ["winged-disc fravahar (later iconography)", "fire as symbol/presence", "the seven Amesha Spentas as his hypostases"]
attested-in: ["[[phase-2-002-gathas-of-zarathustra]]", "[[phase-2-004-yasna-younger-avesta]]", "Achaemenid royal inscriptions (Bisotun / [[darius-i-the-great]]'s Behistun Inscription, Persepolis, Naqsh-e Rustam)"]
related-events: ["[[event-cyrus-conquest-of-babylon-539-bce]]"]
eschatological-agents: ["[[saoshyant]] (the future cosmic-savior whose deeds inaugurate the [[frashokereti-cosmic-renovation]])", "[[sraosha]] (psychopomp at the [[cinvat-bridge-judgment|Cinvat Bridge]])"]
priestly-class: "[[the-magi]]"
equivalents: ["[[varuna]]", "[[yahweh]]", "[[the-one-plotinus]]", "[[brahman-impersonal]]", "[[father-of-greatness]]"]
syncretic-edges:
  - target: "[[varuna]]"
    type: "cognate"
    source: "Thieme 1960; Boyce 1975; West 2007"
    notes: "Both Indo-Iranian sovereign Asura/Ahura-class figures associated with cosmic-moral order. The class shared a stable theological profile (cosmic sovereignty, oath-keeping, truth) preserved on both sides of the IIr religious split"
  - target: "[[yahweh]]"
    type: "substrate-influence"
    source: "Boyce & Grenet 1991 *History of Zoroastrianism* vol. 3; Shaked 1984; Bickerman 1965"
    notes: "Persian-period contact (538-332 BCE Judah as Persian province) — structural and functional parallels between Ahura Mazda's transcendent monotheistic role and the post-exilic Jewish development of YHWH as transcendent sole god. The contemporaneous monotheistic articulations + the Cyrus-as-anointed-of-YHWH passages in Deutero-Isaiah make the contact context concrete"
  - target: "[[the-one-plotinus]]"
    type: "scholarly-parallel"
    source: "Halbfass 1988; the third-century collision-zone (see [[third-century-collision-zone]])"
    notes: "Plotinian One and Zoroastrian Ahura Mazda are both transcendent unitary supreme sources; the 3rd-c. CE collision-zone has them as parallel articulations alongside Manichaean Father of Greatness and Christian God the Father"
  - target: "[[brahman-impersonal]]"
    type: "parallel-motif"
    source: "Comparative IE — Upaniṣadic Brahman and Zoroastrian Ahura Mazda both emerge as transcendent supreme principles from Indo-Iranian polytheistic substrate via reform movements"
    notes: "Two parallel IIr responses to the question of the absolute supreme reality — though Brahman is impersonal (neuter) while Ahura Mazda is personal (masculine)"
  - target: "[[father-of-greatness]]"
    type: "parallel-motif"
    source: "BeDuhn 2000 *The Manichaean Body*; the 3rd-c. collision-zone"
    notes: "Manichaean Father of Greatness explicitly inherits the Zoroastrian Ahura Mazda role as the supreme light-deity in Mani's dualist cosmology. Mani was raised in a Zoroastrian-Christian-Jewish syncretic milieu in Sasanian Mesopotamia; the Father of Greatness is the closest direct theological descent from Ahura Mazda"
  - target: "[[theme-zoroastrian-jewish-exchange]]"
    type: "source-deity-of-transmission"
    source: "Boyce & Grenet 1991, *History of Zoroastrianism* vol. III; Shaked 1984, *Cambridge History of Judaism* vol. 1; Hultgård 1998; Dobroruka 2022 — Ahura Mazda's transcendent-monotheizing profile (uncreated, maximally good, principle of Truth opposed to the Lie) is the Persian-end source-deity for the Achaemenid-period (~539–332 BCE) doctrinal transformation of Second Temple Judaism. See [[theme-zoroastrian-jewish-exchange]] and [[persian-period-injection]] for the full six-channel (dualism / Satan / angelology / demonology / resurrection / messianism) transmission documentation"
    notes: "MASSIVE-WIN: the source-deity of the single most consequential cross-tradition theological exchange in the ancient world — Iran → Judaism → Christianity → Islam"
  - target: "persian-period-injection"
    type: "source-deity-of-transmission"
    source: "Skjærvø 2011 (EIr, AHURA MAZDĀ); Boyce 1982; Collins 1998 — Ahura Mazda's theological architecture (transcendent, ethical-cosmic dualism, divine hypostases as the Amesha Spentas) is the Persian-end of the five-channel injection event documented at [[persian-period-injection]]"
    notes: "Companion edge to theme-zoroastrian-jewish-exchange — persian-period-injection focuses on the full five-channel spectrum with Cyrus-as-mashiach as the instantiation-point"
  - target: "asura-deva-inversion"
    type: "elevated-ahura-class-deity-of-Zoroastrian-branch"
    source: "Skjærvø 1993 EIr 'Daēva'; Boyce 1975 vol. 1 — Ahura Mazda is the *result* of the asura-deva inversion on the Iranian side: the elevation of one *ahura* to supreme status while the *daēva*-class (including the Vedic-cognate Indra) is demonized. The reform's positive move is the full systematization of Ahura Mazda as cosmic-Truth-source; the negative move is the daēva-condemnation. Together they constitute the asura-deva inversion's two faces. See [[asura-deva-inversion]] — also [[persian-period-injection]] for the downstream Second-Temple-Judaism transmission"
    notes: "MASSIVE-WIN: Ahura Mazda IS the asura-deva inversion's positive theological achievement on the Iranian side; without the inversion, no Zoroastrian monotheism-trajectory. Cross-lens: 03_deities → 06_themes"
  - target: "bmac-proto-zoroastrian"
    type: "downstream-deity-of-archaeological-substrate"
    source: "Anthony 2007 *The Horse, the Wheel, and Language* ch. 16; Sarianidi 1998 *Margiana and Protozoroastrianism*; Witzel 1998 — the BMAC (Bactria-Margiana Archaeological Complex ~2200–1700 BCE, modern Turkmenistan/Afghanistan) is the Bronze Age archaeological substrate in which Proto-Indo-Iranian religion crystallized before the Vedic-Avestan split. Ahura Mazda is the eventual Iranian-branch elevation of the *asura/ahura* class that the BMAC contact-zone fostered. See [[bmac-proto-zoroastrian]]"
    notes: "Cross-lens: 03_deities → 06_themes. BMAC documents the *pre-religious* archaeological-substrate context from which both Vedic and Zoroastrian religions emerge — Ahura Mazda is the downstream Iranian-branch endpoint"
  - target: "theme-zoroastrian-cosmogony"
    type: "supreme-creator-of-cosmogony"
    source: "Boyce 1975 *History of Zoroastrianism* vol. I; Zaehner 1955 *Zurvan: A Zoroastrian Dilemma*; Skjærvø 2011 *Zoroastrian Theology from the Gathas to the Achaemenids*; Greater Bundahishn (Anklesaria 1956) — Ahura Mazda is the supreme creator of the developed Bundahishn cosmogony schema: the uncreated Wise Lord who creates the world over twelve millennia in response to Angra Mainyu's invasion. The cosmogony resolves the theological paradox (why did the maximally-good Ahura Mazda create a world Angra Mainyu could invade?) through the Zurvanite framework or through the Gathic free-choice framework. See [[theme-zoroastrian-cosmogony]]"
    notes: "Cross-lens: 03_deities → 06_themes. The Bundahishn cosmogony is the load-bearing Pahlavi-period (~9th–10th c. CE compilation of Sasanian sources) systematization of Gathic + Avestan creation theology"
status: "metadata"
refs:
  - title: "A History of Zoroastrianism, vol. I-III"
    author: "Boyce, Mary (with Grenet for vol. III)"
    year: 1975-1991
    publisher: "Brill"
    type: "monograph"
    tier: 1
  - title: "The Hymns of Zarathustra"
    author: "Insler, S."
    year: 1975
    publisher: "Acta Iranica 8, Brill"
    type: "critical-edition"
    tier: 1
  - title: "Encyclopaedia Iranica: AHURA MAZDĀ"
    author: "Boyce, M."
    publisher: "Encyclopaedia Iranica online"
    url: "https://iranicaonline.org/articles/ahura-mazda"
    type: "encyclopedia"
    tier: 3
tags: [deity, zoroastrian, ahura-mazda, supreme-god, dualism]
---

# Ahura Mazda

## Identity
Ahura Mazda ("Wise Lord", from Avestan *ahura* "lord" + *mazdā* "wise") is the supreme uncreated god of Zoroastrianism. In the Gathas of Zarathustra he is the source of Truth (*aša*) and the Good, opposed by the Lie (*druj*) embodied in Angra Mainyu. Whether the Gathic Ahura Mazda is the *parent* of Spenta Mainyu (the Bounteous Spirit) and Angra Mainyu (the Hostile Spirit) — making him a transcendent first principle above the dualism — or whether Ahura Mazda *is* Spenta Mainyu and is co-eternal with Angra Mainyu, is a foundational interpretive question dividing Zoroastrian theology (Zurvanism took the former view; mainstream Zoroastrianism in dualistic form took the latter).

Ahura Mazda is the chief named god in the Achaemenid royal inscriptions of Darius (Bisotun, ~-520 BCE) and Xerxes — "By the favor of Ahura Mazda I am king" — making him the only god of an early Iron Age world empire that we can name with certainty from contemporary state inscriptions. His worship continued through the Sasanian period (224–651 CE) and survives in the modern Parsi and Iranian Zoroastrian communities. Scholarly parallels to Vedic Varuṇa (Indo-Iranian Asura/Ahura sovereign) and to post-exilic Jewish YHWH (parallel transcendent monotheizing development under Persian conditions) are extensively debated.
