---
type: person
id: "rumi"
name: "Rumi"
aka: ["Jalāl al-Dīn Muḥammad Rūmī", "Mawlānā", "Mevlana (Turkish)", "Jalāl al-Dīn Balkhī"]
role-description: "Sufi master, poet, theologian"
tradition: "Sufi Islam — Mevlevi (Mawlawiyya)"
role-tokens: ["sufi-shaykh", "author-secular", "theologian"]
region: "Khwarazm (birth Balkh), Anatolia (Konya — residence and death)"
date-born: 1207
date-died: 1273
historicity: "documented"
texts-authored: ["[[phase-5-025-rumi-masnavi]]", "*Dīvān-i Shams-i Tabrīzī* (~40,000 lyric verses)", "*Fīhi mā Fīhi* (prose discourses)", "*Majālis-i Sabʿa* (Seven Sermons)", "*Maktūbāt* (letters)"]
mentioned-in: []
hub-edges:
  - target: "soul-exile-longing"
    type: "central-figure-of-cross-tradition-pattern"
    source: "Chittick 1983 *The Sufi Path of Love*; Schimmel 1978 *The Triumphal Sun*; Lewis 2000 *Rumi: Past and Present, East and West* — Rumi is THE most-globally-recognized articulator of the soul-exile-longing pattern. The opening 18 couplets of the Masnavi (the Nay-nāmeh / Reed-flute Song) — 'Listen to the reed, how it tells a tale of separations / Ever since I was cut from the reed bed, man and woman have lamented my complaint' — is the most-cited Sufi formulation of *shawq* (longing) in world literature. The reed cut from the reed bed = the soul cut from God. See [[soul-exile-longing]]"
    notes: "Cross-lens: 04_persons → 06_themes. MASSIVE-WIN: Rumi's Nay-nāmeh is the single most-quoted passage in cross-tradition mysticism — the poetic-prose form of the cross-tradition exile-longing pattern that Plotinus articulates philosophically + Suhrawardī systematizes cosmologically + Vedānta articulates epistemologically"
  - target: "merkabah-miraj-ascent-chain"
    type: "Sufi-articulator-of-mystical-ascent"
    source: "Schimmel 1975 *Mystical Dimensions of Islam*; Schäfer 2009 *Mohammed's Journey to Heaven*; Chittick 1983 — Rumi's Masnavi inherits and elaborates the Sufi Mi'raj-mysticism tradition that descends from Muhammad's Night Journey through Ibn Arabi's *Kitāb al-Isrāʾ* + Suhrawardī's ascent-narratives. Rumi cites the Mi'raj structure throughout the Masnavi as the template for the soul's progressive purification-through-stages. See [[merkabah-miraj-ascent-chain]]"
    notes: "Cross-lens: 04_persons → 06_themes. Rumi is the Sufi-tradition endpoint of the Merkabah → Hekhalot → Mi'raj → Sufi-mystical-ascent chain"
status: "stub"
refs:
  - title: "Rumi: Past and Present, East and West"
    author: "Lewis, Franklin D."
    year: 2000
    publisher: "Oneworld"
    type: "monograph"
    tier: 1
    notes: "Definitive English biography and reception history"
  - title: "The Triumphal Sun: A Study of the Works of Jalaloddin Rumi"
    author: "Schimmel, Annemarie"
    year: 1978
    publisher: "SUNY"
    type: "monograph"
    tier: 1
  - title: "The Sufi Path of Love"
    author: "Chittick, William C."
    year: 1983
    publisher: "SUNY Press"
    type: "monograph"
    tier: 1
tags: [phase-5, islam, sufi, persian-poetry, mevlevi, konya, mathnawi]
---

# Rumi

## Identity
Jalāl al-Dīn Muḥammad Rūmī (1207–1273) — known in the Persianate world as *Mawlānā* ("Our Master") and in Turkey as *Mevlâna* — was the supreme Sufi poet of Persian language and the most-read Persianate religious author of the modern Western world. Born Balkh (modern Afghanistan) 1207; his family fled the Mongol approach and migrated west, eventually settling Konya (Seljuk Anatolian capital) ~1228 where his father Bahāʾ al-Dīn Walad was a prominent Sufi master. Rūmī succeeded his father as community teacher and jurist. The pivotal event of his life was the 1244 encounter with the wandering dervish Shams-i Tabrīzī, a transformative mystical friendship that turned him from learned cleric into ecstatic mystic-poet. After Shams's disappearance (1248 — death by violence is one possibility), the bulk of Rūmī's lyric output (*Dīvān-i Shams-i Tabrīzī*) is dedicated to Shams. The *Masnavi* (~25,000 didactic couplets) belongs to his later period, dictated to Ḥusām al-Dīn Chelebī.

## Historical evidence
Substantial — the *Risāla-yi Sipahsālār* (Sipahsalar's biography of Rūmī's family) and Aflākī's *Manāqib al-ʿĀrifīn* (biographical hagiography of the Mevlevi founders) plus Rūmī's own works.

## Works
*Masnavi* (didactic mystical epic); *Dīvān-i Shams-i Tabrīzī* (lyric *ghazals* and *rubāʿīyāt*); *Fīhi mā Fīhi* (prose discourses); *Majālis-i Sabʿa*; *Maktūbāt*.

## Role in the tradition
After Rūmī's death, his son Sulṭān Walad founded the Mevlevi order; the order spread across the Ottoman Empire and developed the *samāʿ* whirling ritual. Rūmī is the most-read Persian poet in the modern Persianate world (Iran, Afghanistan, Tajikistan, Turkey) and through Coleman Barks's loose English paraphrases became a US bestseller in the 1990s — with significant scholarly critique that the popular American reception strips out the Qur'anic-Islamic substrate.

## Refs
1. Lewis, F.D. (2000). *Rumi: Past and Present, East and West*. Oneworld.
2. Schimmel, A. (1978). *The Triumphal Sun*. SUNY.
3. Chittick, W.C. (1983). *The Sufi Path of Love*. SUNY.
