...
8. **PER-VIEW HIDE-LIST IS FORBIDDEN (2026-05-29).** V01 chrome must be **DELETED** from `index.html` and from `src/js/app.js` in the **same commit** — never hidden via `body.view-X { .legacy-thing { display: none !important; } }`, never stubbed as `.legacy-killswitch` empty divs "for the JS callsites." If you can't delete the markup AND its JS handlers together, you don't ship. Live `index.html` is a minimal V2 shell; each V2 view owns its chrome inside its pane (Forge + Boards do this right). V01 reference lives ONLY at `_legacy/index.html`, never linked from the live tree except via the master-pill "Old prototypes" entry. Verification before reporting completion on ANY shell/UI change: `lsof :8742` (ONE pid of serve-node.js), Cmd-Option-E in Safari to empty caches, hard reload, **screenshot**. The screenshot IS the success criterion — "zero console errors" is not. Full rationale + forbidden-patterns list: `feedback_per_view_hide_list_forbidden_2026-05-29.md` in agent memory. Triggered by 2026-05-29 session where the 2026-05-28 legacy-isolation lock held < 24h before agents reverted to hide-list under a new name (`.legacy-killswitch` stubs).

9. **CHART HULL/FAMILY GROUPING IS A SWAPPABLE PRIMITIVE (2026-05-30).** The Atlas/Forge wheel is the ONE canonical chart per rule #8, but its SPREAD — what becomes a hull, what becomes a wedge — must be a parameterized `groupBy` input the view declares, NOT a hard-coded `n.family` lookup inside `src/js/engine/layout/radial.js` / `src/js/views/forge.js`. Different views feed different grouping primitives: Atlas root → `n.family` (Egyptian/Greek/Norse), Codex/Egyptian → corpus-section (Old Kingdom / Middle Kingdom / New Kingdom / Theological / Greek-Egyptian), Codex/Bible → corpus-section (Pentateuch / Gospels / Pauline / etc.), Lens/Personae → personae-cluster, Lens/Events → era-band. **ONE engine, MANY spreads.** Forbidden: hard-coding `n.family` anywhere in the layout pipeline; building a separate chart engine per grouping scheme; `if (mode === 'scriptures')` special-case branches in the layout primitive; treating corpus-thinness (Egyptian-10-books, Bible-33-books) as an engine problem instead of Lane A content. Full rationale: `feedback_chart_family_swap_is_a_primitive_2026-05-30.md` in agent memory.

11. **NEVER MENU-PICK WHEN A CLEARLY-BETTER PATH EXISTS (2026-05-31).** When you (the agent) see a clearly-better path within a ratified framework or standing protocol, **EXECUTE — do not present a menu.** John's exact words after the Figures-migration v2 trio-audit FAIL: *"yes with your recco. NEVER let me pick if im overlooking"*. This UPGRADES rule #7 (severity dogma) + the `feedback_execute_dont_menu_2026-05-26.md` rule from "in fix mode execute" to ALSO "in plan/diagnostic mode execute" when one path is clearly correct under existing rules.

   **Smell test for fake-fork (do not present menu if any are true):** (1) Is there a clearly-better option among the choices? (2) Is the "better" path under-applied because it's more work or more rigorous? (3) Is the menu hiding a structural call you should have made yourself? (4) Has John already said "same standards" / "max integrity" / "all the way"? If yes to any → execute the integrity-maximizing path, name it, report.

   **Real fork (present menu) vs fake fork (do not):** real fork = multiple genuinely-equivalent paths with trade-offs John must judge (D3 vs vis.js; OAuth vs API key). Fake fork = one path is clearly better under standing rules and you're presenting alternatives because they're cheaper or because you're hedging. The Figures-v2 case ("ship with light fixes" vs "v3 rewrite") was 100% fake-fork — v2 had failed three independent audits; there was no "light fix" path the protocol permitted.

   **What to do instead of menu:** (a) State the call. (b) Name the reasoning (specific rule or memory citation). (c) Execute. (d) Report and invite correction — but don't ask before acting. If John overrides, take the correction; you haven't violated the rule, you've executed under your best understanding of standing instructions.

   **Ratified standing instructions log (do not re-ask):** Tier-1-academic-default + conservative-baseline + T3/T4-marks-alternative-as-alternative + same-standards-max-integrity + execute-don't-menu-pick + never-let-me-pick-if-overlooking. These are settled. Full doctrine: `feedback_never_menu_pick_if_overlooking_2026-05-31.md` in agent memory.

10. **LOOP-DETECTION PROTOCOL (2026-05-31).** If you are about to propose a "Pass-N+1" / "Phase B+1" / "yet-another-gap-audit" iteration of a workstream that's already had ≥3 iterations finding "+N more" of the same kind of gap — **STOP**. You are in a structural-drift loop. The fix is not iteration N+1; the fix is upstream of the iteration cycle.

   **Smell test** — you are looping when ≥3 of these are true: (a) Nth audit of the same shape; (b) each iteration finds "+N more" of the same gap-class; (c) work descriptions say "add N more" not "fix Y"; (d) total work across iterations > cost of structural fix; (e) John says *"this keeps happening"* / *"why is this so hard"* / *"I'm lost"* / *"didn't we already do this"*.

   **What's almost always wrong:** (A) two sources of truth with no mechanical link — hand-maintained list parallel to vault YAML; (B) implicit-rule cascade — a ratified rule applied to the batch in front of you instead of swept across the rule's domain; (C) schema field that's authoritative-in-some-views and stub-in-others; (D) manual allowlist that should be a query over node properties.

   **Resolution:** (1) refuse the next iteration explicitly — *"I'm spotting a loop pattern, let me diagnose why before iteration N+1"*; (2) name the structural cause (which of A–D); (3) propose the structural fix with cost-compared-to-one-more-iteration; (4) get a `go` and ship; (5) document so the loop can't restart (STATUS entry + memory file + cardinal-rule update if warranted).

   **Forbidden patterns:** naming a new batch "Pass-N+1" without justifying why iteration is the right answer; surfacing "apply or not apply" buckets when the question is structural; presenting 3-option forks ("(A) merge / (B) new corpus / (C) exclude") when the underlying issue is two-sources-of-truth; silently repeating an audit shape because "this is just the next pass."

   **Originating example (2026-05-31):** the corpora-catch-up cycle ran 4 iterations (`a3183c87` Phase B / `30898027` Pass-2 / `4e0382ba` integrity audit / `5c117495` final gap) before John called it. The structural fix was the canonical-corpus YAML migration (commit `458da380`) — corpus membership becomes a YAML property of each document; SCRIPTURE_CORPORA dissolves as a hand-maintained list. From iteration-1 the structural cause was identifiable (SCRIPTURE_CORPORA + vault YAML had no mechanical link); should have been caught at iteration 2. Pairs with `feedback_execute_dont_menu_2026-05-26` — this protocol upgrades that rule: don't menu-pick within iteration-N when the real question is whether iteration is the right shape at all. Full doctrine: `feedback_loop_detection_protocol_2026-05-31.md` in agent memory.

12. **MEMBERSHIP IS SINGULAR; REACH IS WIRES (2026-06-02, CARDINAL — read `00_meta/MEMBERSHIP-AND-WIRES.md` before ANY tradition/family/role/wedge work).** Every node belongs to exactly ONE home family — its origin tradition, singular — which alone sets wedge placement (tier-1). Everywhere else a figure appears is an **appearance**: a WIRE (edge) carried by the transmitting doc/author, tagged `appearance-tier: 2` (major) / `3` (minor) by academic prominence in the *receiving* tradition. John's words: *"if a character appears in a family, HE IS THAT FAMILY POINT BLANK PERIOD FULL STOP. associations with other families will be WIRED by AUTHORS or WTVER transmits them."* + *"Christ is always number one in christianity, but in a gnosticism christian wheel he MIGHT appear if the lenses allow — the TIER-one level is not reflected, i.e. an appearance."* **Forbidden:** multi-valued `tradition:` where components cross families; cramming reception into membership; `role-tokens:` carrying a role from a non-home family (that's `role-in-context` on the wire); deleting cross-tradition info instead of moving it to an edge. **Allowed:** within-family synonym labels ("Sumerian / Akkadian"→Mesopotamian — not a violation). **Scripture exception:** singular origin for wedge BUT `canonical-corpus:` stays multi-valued (separate axis — a book is canonical in multiple living canons). `appearance-tier` ≠ `source-tier` ≠ `political-risk-flag` (three orthogonal axes). Full spec + tier table + origin rules + sweep order: `00_meta/MEMBERSHIP-AND-WIRES.md`. Origin crisis: `AUDIT/2026-06-02-MEMBERSHIP-VS-WIRE-CRISIS.md`.
...
| `00_meta/controlled-vocab-registry.yaml` | Adding a new controlled-vocabulary field to a node type OR shipping a new chart/lens that filters on a categorical YAML field. THE schema authority for every active controlled-vocab field. See §"Controlled-vocabulary primitive" in `ONTOLOGY.md` for the pattern. |
| `00_meta/role-vocabulary.yaml` | Classifying a person OR shipping a Lane-A person-node OR contesting a Figures-lens assignment. Anchored in Eliade/Jones 2005 + Smart 1996 + Weber 1922 + Asad 1993. |
| `00_meta/tradition-vocabulary.yaml` | Assigning the `tradition:` field on a new node. Wired into `build_data.py` + `lint_yaml.py validate_traditions()` 2026-05-31. |
| `00_meta/polemical-framing-vocabulary.yaml` | When a figure is opponent-categorized (heresiarch / antichrist / false-prophet / etc.). NEVER use these as primary `role-tokens:` — they go in `polemical-framing:` per `feedback_deviant_bridges_2026-05-16.md`. |
| `00_meta/reclaimed-self-naming-vocabulary.yaml` | When a tradition affirmatively reclaims a polemic-from-outside term (witch, pagan, queer, heretic). |
| `00_meta/role-contested-cases-ratified-2026-05-31.yaml` | Per-person ratified role assignments for cases where the call was contested (Jung, Joseph Smith, Bahá'u'lláh, Wovoka, Sankara, etc.). Single source of truth; the migration script reads from this file. |
...
## 9. The master files (don't edit without permission)

**Tier-1 master files (append-only-with-John's-permission):**
- `00_meta/ONTOLOGY.md`
- `00_meta/PROTOCOL.md`
- `00_meta/LANES.md`
...
**Controlled-vocab schema authorities (added 2026-05-31, append-only-with-John's-permission):**
- `00_meta/controlled-vocab-registry.yaml` — registry of every active controlled-vocab field
- `00_meta/role-vocabulary.yaml` — role-tokens controlled vocabulary
- `00_meta/tradition-vocabulary.yaml` — tradition controlled vocabulary
- `00_meta/polemical-framing-vocabulary.yaml` — opponent-categorization vocabulary
- `00_meta/reclaimed-self-naming-vocabulary.yaml` — affirmative-reclamation vocabulary
- `00_meta/role-contested-cases-ratified-YYYY-MM-DD.yaml` — ratified per-person role assignments (dated files; append-only after sign-off, future overrides via a NEW dated file per the ONTOLOGY-RATIONALE precedent)

These vocabulary files are protected because:
1. Future agents can't silently mutate the controlled vocabulary
2. The schema authority is single-source-of-truth (per rule #10 LOOP-DETECTION PROTOCOL Pattern A — two-sources-of-truth is the structural-drift cause)
3. Adding a new entry requires John's sign-off (same as ONTOLOGY edits)

If you need to add/modify a vocabulary entry, **first ask** (warning rule #4).
